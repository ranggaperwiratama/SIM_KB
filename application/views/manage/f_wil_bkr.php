...
if ($mode == "edit" || $mode == "act_edit") {
	$act		= "act_edit";
	$idp		= $datpil->ID_WILAYAH_BKR;
	$kode_rt				= $datpil->KODE_RT;
	$kode_rw				= $datpil->KODE_RW;
	$kode_kelurahan			= $datpil->KODE_KELURAHAN;
	$kode_kecamatan			= $datpil->KODE_KECAMATAN;
	$nama_kelompok		= $datpil->NAMA_KELOMPOK_BKR;
	$nama_ketua				= $datpil->NAMA_KETUA_BKR;
	$nama_pembina				= $datpil->NAMA_PEMBINA_BKR;
	$tahun_bkr				= $datpil->TAHUN_BKR;
} else {
	$act		= "act_add";
	$idp		= "";
	$kode_rt				= "";
...
?>
<form action="<?=base_URL()?>index.php/manage/wil_bkr/<?=$act?>" method="post">
<input type="hidden" name="idp" value="<?=$idp?>"></input>
	...
	<br>
	<label style="width: 200px; float: left">Nama Kelompok BKR</label><input class="input-xxlarge" type="text" name="nama_kelompok" placeholder="" value="<?=$nama_kelompok?>" required>
	<br>	
	<label style="width: 200px; float: left">Nama Ketua BKR</label><input class="input-xxlarge" type="text" name="nama_ketua" placeholder="" value="<?=$nama_ketua?>" required>
	<br>
	<label style="width: 200px; float: left">Nama Pembina BKR</label><input class="input-xxlarge" type="text" name="nama_pembina" placeholder="" value="<?=$nama_pembina?>" required>
	<br>	
	...
	<input id="tahun" class="input-small" type="hidden" name="tahun_bkr" placeholder="" value="<?=$tahun_bkr?>" required>
	<br>
