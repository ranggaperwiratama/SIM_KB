...
if ($mode == "edit" || $mode == "act_edit") {
	$act		= "act_edit";
	$idp		= $datpil->ID_WILAYAH_BKL;
	$kode_rt				= $datpil->KODE_RT;
	$kode_rw				= $datpil->KODE_RW;
	$kode_kelurahan			= $datpil->KODE_KELURAHAN;
	$kode_kecamatan			= $datpil->KODE_KECAMATAN;
	$nama_kelompok		= $datpil->NAMA_KELOMPOK_BKL;
	$nama_ketua				= $datpil->NAMA_KETUA_BKL;
	$nama_pembina				= $datpil->NAMA_PEMBINA_BKL;
	$tahun_bkl				= $datpil->TAHUN_BKL;
} else {
	$act		= "act_add";
	$idp		= "";
	$kode_rt				= "";
...
?>
<form action="<?=base_URL()?>index.php/manage/wil_bkl/<?=$act?>" method="post">
<input type="hidden" name="idp" value="<?=$idp?>"></input>
	...
	<br>
	<label style="width: 200px; float: left">Nama Kelompok BKL</label><input class="input-xxlarge" type="text" name="nama_kelompok" placeholder="" value="<?=$nama_kelompok?>" required>
	<br>	
	<label style="width: 200px; float: left">Nama Ketua BKL</label><input class="input-xxlarge" type="text" name="nama_ketua" placeholder="" value="<?=$nama_ketua?>" required>
	<br>
	<label style="width: 200px; float: left">Nama Pembina BKL</label><input class="input-xxlarge" type="text" name="nama_pembina" placeholder="" value="<?=$nama_pembina?>" required>
	<br>	
	...
	<input id="tahun" class="input-small" type="hidden" name="tahun_bkl" placeholder="" value="<?=$tahun_bkl?>" required>
	<br>
