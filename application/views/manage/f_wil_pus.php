...
if ($mode == "edit" || $mode == "act_edit") {
	$act		= "act_edit";
	$idp		= $datpil->ID_WILAYAH_PUS;
	$kode_rt				= $datpil->KODE_RT;
	$kode_rw				= $datpil->KODE_RW;
	$kode_kelurahan			= $datpil->KODE_KELURAHAN;
	$kode_kecamatan			= $datpil->KODE_KECAMATAN;
	$nama_kelompok_kb		= $datpil->NAMA_KELOMPOK_KB;
	$nama_ketua				= $datpil->NAMA_KETUA;
	$nama_plkb				= $datpil->NAMA_PLKB;
	$tahun_pus				= $datpil->TAHUN_PUS;
} else {
	$act		= "act_add";
	$idp		= "";
	$kode_rt				= "";
...
?>
<form action="<?=base_URL()?>index.php/manage/wil_pus/<?=$act?>" method="post">
<input type="hidden" name="idp" value="<?=$idp?>"></input>
	<fieldset><legend>Form Pasangan Usia Subur</legend>
	...
	<br>
	<label style="width: 200px; float: left">Nama Kelompok KB</label><input class="input-xxlarge" type="text" name="nama_kelompok_kb" placeholder="" value="<?=$nama_kelompok_kb?>" required>
	<br>	
	<label style="width: 200px; float: left">Nama Ketua</label><input class="input-xxlarge" type="text" name="nama_ketua" placeholder="" value="<?=$nama_ketua?>" required>
	<br>
	<label style="width: 200px; float: left">Nama PLKB</label><input class="input-xxlarge" type="text" name="nama_plkb" placeholder="" value="<?=$nama_plkb?>" required>
	<br>	
	...
	<input id="tahun" class="input-small" type="hidden" name="tahun_pus" placeholder="" value="<?=$tahun_pus?>" required>
	<br>
